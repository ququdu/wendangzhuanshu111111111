...
export class SimilarityChecker {
   ...
   * 检查文本相似度
   */
  async check(text: string, sources: string[], options?: CheckOptions): Promise<SimilarityResult> {
...
${text2.substring(0, 1000)}`
    ...
    const response = await this.providerManager.complete(
      ...
      { providerId: options?.providerId, temperature: 0, maxTokens: 10 }
    )
   ...
   * 在源文本中找到匹配的部分
   */
  private findMatchingSection(chunk: string, source: string): string {
