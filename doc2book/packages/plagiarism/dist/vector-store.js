...
class VectorStore {
     ...
     * 检查文本是否与存储中的内容相似
     */
    async checkSimilarity(text, threshold) {
...
文本：${text.substring(0, 500)}`;
        try {
            const response = await this.providerManager.complete([{ role: 'user', content: prompt }], { temperature: 0, maxTokens: this.dimensions * 10 });
