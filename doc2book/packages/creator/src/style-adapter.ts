...
export class StyleAdapter {
  ...
  async adapt(content: string, targetStyle: StyleOptions, options?: { providerId?: string }): Promise<{
    success: boolean
    error?: string
...
只输出调整后的内容，不要解释。`
      ...
      const response = await this.providerManager.complete(
        ...
        { providerId: options?.providerId, temperature: 0.5, maxTokens: 4000 }
      )
  ...
  private describeStyle(style: StyleOptions): string {
    const parts: string[] = []
